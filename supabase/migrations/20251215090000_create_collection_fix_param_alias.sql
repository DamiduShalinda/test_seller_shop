-- so they never shadow table columns when used in SQL expressions. Also re-apply the
-- quantity guard logic to ensure we keep existing constraints.

drop function if exists public.create_collection(uuid, integer);

create or replace function public.create_collection(
  p_batch_id uuid,
  p_collected_quantity integer
)
...
begin
  ...
  if p_collected_quantity is null or p_collected_quantity <= 0 then
  ...
  if v_existing_total + p_collected_quantity > v_batch.quantity then
